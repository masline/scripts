...
DEBIAN_FRONTEND=noninteractive aptitude install -y -q python3.4-dev python-pip

# Replace python2 with python3 as symlinked python
ln -sf /usr/bin/python3.4 /usr/bin/python

# Install some newer system packages
DEBIAN_FRONTEND=noninteractive aptitude install -y -q build-essential libtext-charwidth-perl libuuid-perl liblocale-gettext-perl libtext-iconv-perl libxml2-dev libssl-dev libjpeg8-dev libpng12-dev libfreetype6-dev libldap2-dev libmcrypt-dev libxslt-dev
